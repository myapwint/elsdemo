...
public final class SearchRequestBuilder {
    ...
    public static SearchRequest from(SearchPayload<?> payload) {
        ...
        List<SortOptions> sorters = payload.getSorters() == null ? List.of() : payload.getSorters().stream().map(SearchRequestBuilder::sortOption).toList();
    ...
    private static Query buildQuery(List<FilterCriteria> filters) {
        ...
        filters.stream().map(SearchRequestBuilder::query).forEach(bool::filter);
        return bool.build();
    }
    ...
    private static Query query(FilterCriteria filter) {
        ...
        return switch (normalizedType(filter.getType())) {
            ...
            case "match_phrase", "phrase" ->
                    ElasticsearchQueryBuilder.matchPhrase(filter.getField(), stringValue(filter.getValue(), "value"));
            case "prefix" ->
                    ElasticsearchQueryBuilder.prefix(filter.getField(), stringValue(filter.getValue(), "value"));
            case "wildcard" ->
                    ElasticsearchQueryBuilder.wildcard(filter.getField(),"*" +  stringValue(filter.getValue(), "value") + "*"  );
    ...
    private static Query rangeQuery(FilterCriteria filter) {
        ...
        return ElasticsearchQueryBuilder.termRange(filter.getField(), from == null ? null : stringValue(from, "from"), to == null ? null : stringValue(to, "to"));
    }

    private static SortOptions sortOption(SorterCriteria sorter) {
        Objects.requireNonNull(sorter, "sorter must not be null");

        return SortOptions.of(sort -> sort.field(field -> field.field(requireText(sorter.getField(), "sorter.field")).order(sortOrder(sorter.getDirection()))));
    }
